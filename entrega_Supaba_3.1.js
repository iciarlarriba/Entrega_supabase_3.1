...
//  Supabase 
const supabaseUrl = 'url de supabase';
const supabaseKey = 'ingresar contraseña de supabase';
...
async function setupSupabase() {
    try {
        ...
        console.error('Error configuración de Supabase:', error.message);
    }
}
...
const newView = () => {
    return `<h2>Crear Película</h2>
        ...
        <div class="field">
        Miniatura <br>
        <input  type="text" id="miniatura" placeholder="URL miniatura">
        </div>
        <div class="actions">
            <button class="create">
                Crear
...
const deleteContr = async (i) => {
    try {
        let confirmacion = confirm("¿ quieres eliminar esta película?");
        if (confirmacion) {
            const { data: peliculas, error } = await supabase.from('peliculas').select('*');
...
const resetContr = async () => {
    try {
        let confirmacion = confirm("¿ quieres restaurar las películas originales?");
        if (confirmacion) {
            await supabase.from('peliculas').upsert(mis_peliculas_iniciales);
            indexContr();
        }
    } catch (error) {
        console.error('Error películas originales:', error.message);
    }
};
